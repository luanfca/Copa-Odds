  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-1 sm:px-4 py-2">
      ...
      <div className="relative overflow-hidden rounded-[28px] border border-white/[0.06] p-6 sm:p-8">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/[0.08] border border-amber-500/[0.12] text-[9px] font-black uppercase tracking-[0.2em] text-amber-500/80">
          ...
          {!loading && !error && players.length > 0 && (
            <div className="flex gap-3">
              ...
              <div className="flex items-center gap-3 px-4 py-3 rounded-2xl border border-white/[0.04] bg-white/[0.015]">
                ...
                <div>
                  <div className="text-lg font-black text-orange-400 tracking-tight">{stats.maxAvg.toFixed(1)}</div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                <label className="text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground/40 whitespace-nowrap">
                  Média min: <span className="text-amber-400/80 font-mono">{minAvg.toFixed(1)}</span>
                </label>
                <input
                  type="range"
                  min="0"
                  ...
                  className="w-24 h-1 bg-white/[0.06] rounded-lg appearance-none cursor-pointer accent-amber-500"
                />
              </div>
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-4 rounded-3xl border border-white/[0.04] bg-white/[0.01]">
          <Footprints className="w-10 h-10 text-muted-foreground/20" />
          ...
          {history ? (
            <div className="flex items-center gap-1.5">
              <span className={cn(
                ...
                history.average >= lineTarget ? 'text-amber-400' : 'text-foreground/70'
              )}>
                ...
                {value !== undefined ? (
                  <a
                    ...
                    className={cn(
                      ...
                      isBest
                        ? 'bg-amber-500/12 border-amber-500/45 text-amber-400 shadow-sm shadow-amber-500/10'
                        : 'bg-white/[0.02] border-white/5 text-foreground/80 hover:bg-amber-500/5 hover:border-amber-500/25 hover:-translate-y-0.5'
                    )}
                  >
                    <span className="leading-none">{formatOdd(value)}</span>
                    {isBest && (
                      <span className="text-[7px] font-black uppercase tracking-widest text-amber-400/80 mt-0.5 leading-none">
                        ★ best
                      </span>
                    )}
                  </a>
        ...
        <div className="flex items-center justify-end gap-2 lg:pl-2">
          {bestOdd && bestOdd.line === line ? (
            <div className="flex flex-col items-end">
              <span className="font-mono font-black text-amber-400 text-base tracking-tight">
            ...
            <div className="rounded-xl border border-white/[0.04] bg-white/[0.01] overflow-hidden">
              <div className="flex items-center gap-2 px-3 py-2 border-b border-white/[0.04]">
                ...
                <span className="text-[9px] font-bold text-muted-foreground/50 ml-auto">
                  Média: <span className="text-amber-400 font-mono">{history.average.toFixed(1)}</span>
                </span>
              </div>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 px-3 py-2 text-xs">
                      ...
                      <span className={cn(
                        ...
                        hit ? 'text-amber-400' : 'text-foreground/50'
                      )}>
                        {e.value}
                      </span>
                      {hit && (
                        <span className="text-[8px] font-bold text-amber-400/60 uppercase">✓</span>
                      )}
                    </div>
                  );
                })}
