...
import {
  AlertTriangle, RefreshCw, Search, Trophy, ChevronDown, ChevronUp,
...
const MARKETS = [
  { id: 'faltas_cometidas', label: 'Faltas Cometidas', icon: AlertTriangle, color: 'text-rose-400', description: 'Faltas cometidas pelo jogador' },
  { id: 'faltas_sofridas', label: 'Faltas Sofridas', icon: AlertTriangle, color: 'text-pink-400', description: 'Faltas sofridas pelo jogador' },
] as const;

type MarketId = typeof MARKETS[number]['id'];

const LINES: Record<MarketId, string[]> = {
  faltas_cometidas: ['1+', '2+', '3+', '4+'],
  faltas_sofridas: ['1+', '2+', '3+', '4+'],
};
...
export default function FaltasPage() {
  const [activeMarket, setActiveMarket] = useState<MarketId>('faltas_cometidas');
  ...
  function handleMarketChange(market: MarketId) {
    ...
    if (!LINES[market].includes(selectedLine)) {
      setSelectedLine(LINES[market][0] ?? '2+');
    }
      ...
      <div className="relative overflow-hidden rounded-[28px] border border-white/[0.06] p-6 sm:p-8">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-500/[0.08] border border-rose-500/[0.12] text-[9px] font-black uppercase tracking-[0.2em] text-rose-400/80">
              <AlertTriangle className="w-3 h-3" />
              Ranking de Faltas
            </div>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight">
              <span className="text-foreground/80">MELHORES </span>
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-rose-400 via-pink-400 to-fuchsia-500">FALTAS</span>
            </h1>
            <p className="text-xs text-muted-foreground/50 max-w-md leading-relaxed">
              Jogadores ranqueados por faltas cometidas e sofridas, com as melhores odds disponíveis.
            </p>
      ...
      {!loading && !error && uniqueMatches.length > 0 && (
        <div className="max-w-md">
          {(() => {
            ...
            if (!m) return null;
            return <SofaScoreStats homeTeam={m.homeTeam} awayTeam={m.awayTeam} date={m.dateTime?.slice(0, 10)} highlight={activeMarket === 'faltas_cometidas' ? 'faltas_cometidas' : 'faltas_sofridas'} />;
          })()}
        </div>
      )}
      ...
      {!loading && !error && (
        <div className="p-4 rounded-2xl border border-white/[0.04] bg-white/[0.015]">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            ...
            {/* Histórico: escopo + nº de jogos */}
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground/40 flex items-center gap-1.5">
                <Trophy className="w-3 h-3" /> Histórico
                {historyLoading && (
                  <span className="ml-1 text-[9px] font-bold text-rose-400 animate-pulse normal-case tracking-normal">
                    carregando…
                  </span>
                )}
              </label>
          ...
          <div className="flex flex-wrap gap-2 mb-4 pb-4 border-b border-white/[0.04]">
            ...
            <button
              onClick={() => setSelectedCompetition('premier_league')}
              className={cn(
                'px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all border',
            ...
            <button
              onClick={() => setSelectedCompetition('la_liga')}
              className={cn(
                'px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all border',
                selectedCompetition === 'la_liga'
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mt-4 pt-3 border-t border-white/[0.04]">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-24 h-1 bg-white/[0.06] rounded-lg appearance-none cursor-pointer accent-rose-400"
                />
              </div>
            ...
            <button
              onClick={() => { invalidateMarket(`faltas_${activeMarket}_${selectedCompetition}`); load(true); }}
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {isStarter && (
                <span title="Provável titular" className="text-rose-400 shrink-0 text-[9px] font-black uppercase tracking-wider">
                  XI
                </span>
              )}
